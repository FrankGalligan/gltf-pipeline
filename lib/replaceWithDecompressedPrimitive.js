    ...
    for (const semantic in dracoAttributes) {
        if (Object.prototype.hasOwnProperty.call(dracoAttributes, semantic)) {
            ...
            if (uncompressedFallback) {
                ...
                const attributeId = dracoAttributes[semantic];
                checkMinMaxFromDecode(decoderModule, dracoEncodedBuffer, attributeId, semantic, minmax);
            }
        }
    }
...
function checkMinMaxFromDecode(decoderModule, dracoEncodedBuffer, attributeId, semantic, minmax) {
    let decoder = new decoderModule.Decoder();
    let dracoGeometry = decompressDracoBuffer(decoder, dracoEncodedBuffer.buffer);
    ...
    for (let p = 0; p < numPoints; p++) {
        for (let c = 0; c < numComponents; c++) {
            const index = (p * numComponents) + c;
            const value = attributeData.GetValue(index);
            if (value < minmax.min[c]) {
              console.log('attributeId: ' + attributeId + '  ' + semantic + '  value < minmax.min[' + c + '] ' + value + ' < '  + minmax.min[c]);
            }
            if (value > minmax.max[c]) {
              console.log('attributeId: ' + attributeId + '  ' + semantic + '  value > minmax.max[' + c + '] ' + value + ' < ' + minmax.max[c]);
            }
        }
    }
