...
// From uninterleaveAndPackBuffers.js
/*
...
// values = Array of VECN?
// type = type of |values|
// output = Array of floats
function packArray(values, type) {
    const length = values.length;
    const numComponents = numberOfComponentsForType(type);
    var outArray = [];

    for (var i = 0; i < length; ++i) {
        for (var j = 0; j < numComponents; ++j) {
            outArray.push(values[i][j]);
        }
    }
    return outArray;
}
...
function compressDracoAnimations(gltf, options) {
    ...
    for (var input in extractedAnimations) {
      ...
      //var timestampsData = readAccessor(gltf, gltf.accessors[input]);
      ...
      outputs.forEach(function (output) {
        ...
        //var values = readAccessor(gltf, gltf.accessors[output]);
        var values = readAccessorPacked(gltf, gltf.accessors[output]);
        const numComponents = numberOfComponentsForType(gltf.accessors[output].type);
        const packed = packArray(values, gltf.accessors[output].type);
