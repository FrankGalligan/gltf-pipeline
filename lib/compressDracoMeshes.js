    ...
    ForEach.mesh(gltf, function(mesh) {
        ForEach.meshPrimitive(mesh, function(primitive) {
            ...
            ForEach.meshPrimitiveAttribute(primitive, function(accessorId, semantic) {
                ...
                var dracoAttributeType = encoderModule[attributeName];
                if (semantic === 'TEXCOORD_0') {
                    dracoAttributeType = encoderModule.TEX_COORD;
                } else if (attributeName !== 'POSITION' && attributeName !== 'NORMAL' &&
                    attributeName !== 'COLOR') {
                    dracoAttributeType = encoderModule.GENERIC;
                }

                var attributeId = -1;
                if (attributeData.componentType === 5120) {
                    var data = new Int8Array(attributeData.data);
                    attributeId = meshBuilder.AddInt8Attribute(newMesh, dracoAttributeType,
                        numberOfPoints, attributeData.numberOfComponents, data);
                } else if (attributeData.componentType === 5121) {
                    var data = new Uint32Array(attributeData.data);
                    attributeId = meshBuilder.AddUInt8Attribute(newMesh, dracoAttributeType,
                        numberOfPoints, attributeData.numberOfComponents, data);
                } else if (attributeData.componentType === 5122) {
                    var data = new Int16Array(attributeData.data);
                    attributeId = meshBuilder.AddInt16Attribute(newMesh, dracoAttributeType,
                        numberOfPoints, attributeData.numberOfComponents, data);
                } else if (attributeData.componentType === 5123) {
                    var data = new Uint16Array(attributeData.data);
                    attributeId = meshBuilder.AddUInt16Attribute(newMesh, dracoAttributeType,
                        numberOfPoints, attributeData.numberOfComponents, data);
                } else if (attributeData.componentType === 5125) {
                    var data = new Uint32Array(attributeData.data);
                    attributeId = meshBuilder.AddUInt32Attribute(newMesh, dracoAttributeType,
                        numberOfPoints, attributeData.numberOfComponents, data);
                } else {
                    var data = new Float32Array(attributeData.data);
                    attributeId = meshBuilder.AddFloatAttribute(newMesh, dracoAttributeType,
                        numberOfPoints, attributeData.numberOfComponents, data);
                }
