...
function decmopressDracoMesh(gltf, primitive, decoderModule, decoder, geometryType, dracoBuffer, dracoExtension) {
    ...
    if (defined(dracoExtension.targets)) {
        for (var targetIndex = 0; targetIndex < dracoExtension.targets.length; ++targetIndex) {
            var target = dracoExtension.targets[targetIndex];
            for (var semantic in target) {
                if (target.hasOwnProperty(semantic)) {
                    console.log('semantic:' + semantic + '  target[semantic]:' + target[semantic]);

                    var attribute = decoder.GetAttributeByUniqueId(dracoGeometry, target[semantic]);
                    var attributeBuffer = getAttributeBuffer(gltf, decoderModule, decoder, dracoGeometry, attribute);
                    var bufferViewId = addBufferToGltf(gltf, attributeBuffer);
                    var gltfAttributeId = primitive.targets[targetIndex][semantic];
                    var attributeAccessor = gltf.accessors[gltfAttributeId];
