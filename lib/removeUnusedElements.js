...
Remove.accessor = function(gltf, accessorId) {
    ...
    if (hasExtension(gltf, 'Draco_animation_compression')) {
        var numAnimations = gltf.extensions['Draco_animation_compression'].length;
        //console.log('Remove accessor from Draco_animation_compression!!!  numAnimations:' + numAnimations.toString());
        for (var i = 0; i < numAnimations; i++) {
            var animation = gltf.extensions['Draco_animation_compression'][i];
            animation.input--;
            for (var j = 0; j < animation.outputs.length; j++) {
                animation.outputs[j]--;
            }
        }
    }
...
getListOfElementsIdsInUse.accessor = function(gltf) {
    ...
    if (hasExtension(gltf, 'Draco_animation_compression')) {
        ...
        for (var i = 0; i < numAnimations; i++) {
            var animation = gltf.extensions['Draco_animation_compression'][i];
            usedAccessorIds[animation.input] = true;
            for (var j = 0; j < animation.outputs.length; j++) {
                usedAccessorIds[animation.outputs[j]] = true;
            }
        }
    }
...
getListOfElementsIdsInUse.bufferView = function(gltf) {
    ...
    if (hasExtension(gltf, 'Draco_animation_compression')) {
        ...
        for (var i = 0; i < numAnimations; i++) {
            var animation = gltf.extensions['Draco_animation_compression'][i];
            usedBufferViewIds[animation.bufferView] = true;
        }
    }
