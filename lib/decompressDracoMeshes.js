...
*/
                ...
                var bufferView = dracoExtension.bufferView;
                var buffer = gltf.buffers[bufferView];
                var source = buffer.extras._pipeline.source;
                var compressedData = new Uint8Array(source.buffer);
		...
		//removeExtension(gltf, primitive, primitive.extensions.KHR_draco_mesh_compression);
	    }
        });
    });
...
function getIndicesBuffer(gltf, decoderModule, decoder, dracoGeometry, numFaces) {
    // Convert indices
...
function getAttributeBuffer(gltf, decoderModule, decoder, dracoGeometry, attribute) {
    ...
    console.log('numPoints:' + numPoints + '  numValues:' + numValues);
...
*/
	...
	var indicesBuffer = getIndicesBuffer(gltf, decoderModule, decoder, dracoGeometry, numFaces);

/*
	//var indicesUint8Data = new Buffer.from(indices.buffer);
	var encodedLength = indicesBuffer.length;
        var buffer = {
            byteLength : encodedLength,
            extras : {
                _pipeline : {
...
*/
    ...
    for (var semantic in attributes) {
	    ...
	    console.log('semantic:' + semantic + '  attributes[semantic]:' + attributes[semantic]);
            ...
            var attribute = decoder.GetAttribute(dracoGeometry, attributes[semantic]);
...
/*
	    console.log('attribute type:' + attribute.attribute_type() + '  data_type:' + attribute.data_type());
            ...
            var attributeData;
            if (attribute.data_type() === 5) {
                attributeData = new decoderModule.DracoInt32Array();
                var gotData = decoder.GetAttributeInt32ForAllPoints(
                    dracoGeometry, attribute, attributeData);
                //geometryBuffer[attributeName] = new Int32Array(numValues);
                console.log('decoder.GetAttributeInt32ForAllPoints() :' + gotData + '  size():' + attributeData.size());
            } else {
                attributeData = new decoderModule.DracoFloat32Array();
                var gotData = decoder.GetAttributeFloatForAllPoints(
                    dracoGeometry, attribute, attributeData);
                //geometryBuffer[attributeName] = new Float32Array(numValues);
                console.log('decoder.GetAttributeFloatForAllPoints() :' + gotData + '  size():' + attributeData.size());
            }
