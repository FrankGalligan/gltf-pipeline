...
function compressDracoMeshes(gltf, options) {
    ...
    var compressedMorphTarget = false;
        ...
        ForEach.meshPrimitive(mesh, function(primitive) {
            ...
            ForEach.meshPrimitiveTarget(primitive, function(target) {
                ...
                ForEach.meshPrimitiveTargetAttribute(target, function(accessorId, semantic) {
                    ...
                    compressedTarget[semantic] = attributeId;
                });
                compressedTargetList.push(compressedTarget);
            });

            if (compressedTargetList.length > 0) {
                compressedMorphTarget = true;
            ...
            addCompressionExtensionToPrimitive(gltf, primitive, attributeToId, compressedTargetList, dracoEncodedBuffer, uncompressedFallback);
    ...
    if (addedExtension) {
        ...
        if (compressedMorphTarget) {
            addExtensionsRequired(gltf, 'DRACO_morph_target_compression');
        }
...
function addCompressionExtensionToPrimitive(gltf, primitive, attributeToId, compressedTargetList, dracoEncodedBuffer, uncompressedFallback) {
    if (!uncompressedFallback) {
        ...
        ForEach.meshPrimitiveTarget(primitive, function(target) {
            ForEach.meshPrimitiveTargetAttribute(target, function(accessorId, semantic) {
                ...
                delete attributeAccessor.bufferView;
                delete attributeAccessor.byteOffset;
                target[semantic] = addToArray(gltf.accessors, attributeAccessor);
