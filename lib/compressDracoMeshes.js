...
function compressDracoMeshes(gltf, options) {
    ...
    var allMaxNumberPrimitiveAttributes = 0;
    var totalVertsWritten = 1;
    var posBufferAll = Buffer.alloc(0);
    var normBufferAll = Buffer.alloc(0);
    var texBufferAll = Buffer.alloc(0);
    var faceBufferAll = Buffer.alloc(0);
    ...
    ForEach.mesh(gltf, function(mesh) {
        ...
        ForEach.meshPrimitive(mesh, function(primitive) {
            ...
            ForEach.meshPrimitiveAttribute(primitive, function(accessorId, semantic) {
                ...
                if (attributeName === 'POSITION') {
                    ...
                    // Save pos buffer for all primitives of current mesh.
                    var newLength = posBuffer.length + posBufferMesh.length;
                    posBufferMesh = Buffer.concat([posBufferMesh, posBuffer], newLength);

                    // Save pos buffer for all meshes and primitives.
                    newLength = posBuffer.length + posBufferAll.length;
                    posBufferAll = Buffer.concat([posBufferAll, posBuffer], newLength);
                } else if (attributeName === 'TEXCOORD') {
                    texBuffer = outputAttribute(numberOfPoints, numberOfComponents, 'vt ', data);

                    // Save tex buffer for all primitives of current mesh.
                    var newLength = texBuffer.length + texBufferMesh.length;
                    texBufferMesh = Buffer.concat([texBufferMesh, texBuffer], newLength);

                    // Save tex buffer for all meshes and primitives.
                    newLength = texBuffer.length + texBufferAll.length;
                    texBufferAll = Buffer.concat([texBufferAll, texBuffer], newLength);
                } else if (attributeName === 'NORMAL') {
                    normBuffer = outputAttribute(numberOfPoints, numberOfComponents, 'vn ', data);

                    // Save norm buffer for all primitives of current mesh.
                    var newLength = normBuffer.length + normBufferMesh.length;
                    normBufferMesh = Buffer.concat([normBufferMesh, normBuffer], newLength);

                    // Save norm buffer for all meshes and primitives.
                    newLength = normBuffer.length + normBufferAll.length;
                    normBufferAll = Buffer.concat([normBufferAll, normBuffer], newLength);
            ...
            if (maxNumberPrimitiveAttributes > 0 &&
              ...
              // Record max number of attributes for writing of face buffers.
              // This will work if next primitives have <= num attributes.
              maxNumberPrimitiveAttributes = numberOfAttributesToOutput;
            }

            if (allMaxNumberPrimitiveAttributes > 0 &&
                allMaxNumberPrimitiveAttributes < numberOfAttributesToOutput) {
              console.log('Warning: Obj file will most likely be broken.');
              console.log('allMaxNumberPrimitiveAttributes: ' + allMaxNumberPrimitiveAttributes.toString() +
                          '< numberOfAttributesToOutput: ' + numberOfAttributesToOutput.toString());
            } else if (allMaxNumberPrimitiveAttributes == 0) {
              // Record max number of attributes for writing of face buffers.
              // This will work if next primitives have <= num attributes.
              allMaxNumberPrimitiveAttributes = numberOfAttributesToOutput;
            }
            ...
            var subobjName = 'mesh-' + meshIndex.toString() + '_primitive-' + primitiveIndex.toString();
            var subobjFaceBuffer = outputFaces2(numberOfFaces, attributesOutputList, 'f ',
                                                indices, totalNumberOfMeshVertsWritten, subobjName);

            // Save face buffer for all primitives of current mesh.
            ...
            subobjFaceBuffer = outputFaces2(numberOfFaces, attributesOutputList, 'f ',
                                            indices, totalVertsWritten, subobjName);

            // Save face buffer for all meshes and primitives.
            newLength = subobjFaceBuffer.length + faceBufferAll.length;
            faceBufferAll = Buffer.concat([faceBufferAll, subobjFaceBuffer], newLength);

            totalVertsWritten += lastNumberOfPoints;
        ...
        if (!options.dracoOptions.gltf2ObjOutputOneObj) {
          ...
          objNameMesh += '/' + options.dracoOptions.gltf2ObjPrefix;
          objNameMesh += '_gltf_mesh-' + meshIndex.toString() + '.obj';
          fsExtra.outputFile(objNameMesh, objBufferMesh, undefined);
        }
    });

    if (options.dracoOptions.gltf2ObjOutputOneObj) {
      var totalLength = posBufferAll.length + texBufferAll.length + normBufferAll.length + faceBufferAll.length;
      var objBufferAll = Buffer.concat([posBufferAll, texBufferAll, normBufferAll, faceBufferAll], totalLength);
      var objNameMesh = options.dracoOptions.gltf2ObjDirectory;
      objNameMesh += '/' + options.dracoOptions.gltf2ObjPrefix + '_gltf_mesh.obj';
      fsExtra.outputFile(objNameMesh, objBufferAll, undefined);
    }
