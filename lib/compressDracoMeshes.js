...
function compressDracoMeshes(gltf, options) {
    ...
    ForEach.mesh(gltf, function(mesh) {
        ...
        ForEach.meshPrimitive(mesh, function(primitive) {
            ...
            var posData = undefined;
            var posComponentType = 0;
            var posComponents = 0;
            var posPoints = 0;
            ...
            ForEach.meshPrimitiveAttribute(primitive, function(accessorId, semantic) {
                ...
                if (attributeName === 'POSITION') {
                    posData = data;
                    posComponentType = componentType;
                    posComponents = numberOfComponents;
                    posPoints = numberOfPoints;
                }
            ...
            if (defined(posData)) {
                outputObjPositions(objName, posComponentType, posPoints, posComponents, posData);
            }
...
function outputObjPositions(objFile, componentType, points, components, data) {
    console.log('points:' + points.toString());
    console.log('components:' + components.toString());
    console.log('componentType:' + componentType.toString());

    /*
    fsExtra.outputFile(objFile, '# Positions', undefined);
    for (var p = 0; p < points; ++p) {
        for (var c = 0; c < components; ++c) {
            //fsExtra.outputFile(objFile, 'v ', undefined);
            fs.appendFileSync(objFile, 'v ');
            var index = (p * components) + c;
            //fsExtra.outputFile(objFile, data[index], undefined);
            //fsExtra.outputFile(objFile, ' ', undefined);
            fs.appendFileSync(objFile, data[index]);
            fs.appendFileSync(objFile, ' ');
...
*/
    ...
    var bytesWritten = buf.write('# Positions');
    // fsExtra.outputFile(objFile, '# Positions', undefined);

    for (var p = 0; p < points; ++p) {
        bytesWritten += buf.write('v ');

        for (var c = 0; c < components; ++c) {
            var index = (p * components) + c;
            bytesWritten += buf.write(data[index], 0);
        }
    }

    fsExtra.outputFile(objFile, buf, undefined);
}
